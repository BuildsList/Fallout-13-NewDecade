	...
	switch(slot)
		...
		if(slot_s_store)
			...
			if(I.w_class > WEIGHT_CLASS_BULKY)
				...
				return 0
			if( istype(I, /obj/item/clothing/gloves/pda) || istype(I, /obj/item/weapon/pen) || is_type_in_list(I, H.wear_suit.allowed) )
				return 1
			return 0
		if(slot_handcuffed)
			if(H.handcuffed)
...
/datum/species/proc/spec_attacked_by(obj/item/I, mob/living/user, obj/item/bodypart/affecting, intent, mob/living/carbon/human/H/*, dammod*/)
	...
	if(def_zone == "r_arm" || def_zone == "l_arm")
		apply_damage(round(I.force* 0.6 * rand(6, 14)/10,1), I.damtype, def_zone, armor_block, H)

	if(def_zone == "r_leg" || def_zone == "l_leg")
		apply_damage(round(I.force* 0.6 * rand(6, 14)/10,1), I.damtype, def_zone, armor_block, H)

	if(def_zone == "head")
		apply_damage(round(I.force* 2 * rand(6, 14)/10,1), I.damtype, def_zone, armor_block, H)

	else
		apply_damage(round(I.force*rand(6, 14)/10,1), I.damtype, def_zone, armor_block, H)
