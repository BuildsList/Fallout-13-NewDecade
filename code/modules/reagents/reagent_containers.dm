	...
	if(possible_transfer_amounts.len)
		...
		for(var/A in possible_transfer_amounts)
			...
			if(A == amount_per_transfer_from_this)
				...
				to_chat(user, "<span class='notice'>[src] теперь переливает [amount_per_transfer_from_this] едениц.</span>")
				return
...
/obj/item/weapon/reagent_containers/proc/canconsume(mob/eater, mob/user)
	...
	if(covered)
		...
		to_chat(user, "<span class='warning'>Снимите [who] [covered] для начала!</span>")
		return 0
	return 1
