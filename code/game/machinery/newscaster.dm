...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
			...
			while (findtext_char(channel_name," ") == 1)
				channel_name = copytext_char(channel_name,2,length(channel_name)+1)
			updateUsrDialog()
			...
			var/cominput = copytext_char(stripped_input(usr, "Write your message:", "New comment", null),1,141)
			if(cominput)
				scan_user(usr)
...
/obj/machinery/newscaster/proc/scan_user(mob/living/user)
	if(ishuman(user))
		...
		if(human_user.wear_id)
			if(istype(human_user.wear_id, /obj/item/clothing/gloves/pda))
				var/obj/item/clothing/gloves/pda/P = human_user.wear_id
				if(P.id)
